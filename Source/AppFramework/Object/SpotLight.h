...
{
    class SpotLight : public SObject
    {
    public:
        ...
        TransformComponent& Transform()
        {
            ...
            assert(component && "SpotLight requires TransformComponent.");
            return *component;
        }
        ...
        float InnerAngle() const { return m_innerAngle; }
        void SetInnerAngle(float value) { m_innerAngle = value; }

        float OuterAngle() const { return m_outerAngle; }
        void SetOuterAngle(float value) { m_outerAngle = value; }

        float* ColorData() { return m_color.Data(); }
        const float* ColorData() const { return m_color.Data(); }

        RenderSpotLight BuildRenderLightProxy() const
        {
            ...
            out.range = m_range;
            out.yaw = transform.rotation.yaw;
            out.pitch = transform.rotation.pitch;
            out.color[0] = m_color.r;
            out.color[1] = m_color.g;
            out.color[2] = m_color.b;
            out.intensity = m_intensity;
            out.innerAngle = m_innerAngle;
            out.outerAngle = m_outerAngle;
            return out;
        }
