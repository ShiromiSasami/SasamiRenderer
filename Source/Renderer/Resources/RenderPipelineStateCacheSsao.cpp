// RenderPipelineStateCacheSsao.cpp
...
namespace SasamiRenderer
{
                                                            ...
                                                            const std::string& pixelProfile)
    {
        ...
        D3D12_STATIC_SAMPLER_DESC ssaoSampler = MakePointClampSampler();
        ...
        if (!LoadShaderBlob(m_shaderBlobCache, L"Effects/AmbientOcclusion/SSAO/SSAO_VS.hlsl",      "VSMain", vertexProfile.c_str(), ssaoVS))      return false;
        if (!LoadShaderBlob(m_shaderBlobCache, L"Effects/AmbientOcclusion/SSAO/SSAO_PS.hlsl",      "PSMain", pixelProfile.c_str(),  ssaoPS))      return false;
        if (!LoadShaderBlob(m_shaderBlobCache, L"Effects/AmbientOcclusion/SSAO/SSAO_Blur_PS.hlsl", "PSMain", pixelProfile.c_str(),  ssaoBlurPS))  return false;
        ...
        D3D12_STATIC_SAMPLER_DESC blurSampler = MakePointClampSampler();
