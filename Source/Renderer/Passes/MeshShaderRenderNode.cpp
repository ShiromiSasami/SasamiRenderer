#define NOMINMAX
#include "Renderer/Passes/MeshShaderRenderNode.h"
...
{
    void MeshShaderRenderNode::BuildRequirements(RenderNodeRequirementBuilder& builder) const
    {
        builder.RequireGraphicsBase();
    }

    void MeshShaderRenderNode::Setup(RenderGraphBuilder& builder) const
    {
    ...
    bool MeshShaderRenderNode::Execute(const RenderNodeContextView& context) const
    {
        if (!context.IsSatisfied()) {
            DebugLog("MeshShaderRenderNode::Execute: runtime context is invalid.\n");
            return false;
        }
        ...
        // overload is a no-op — the node is registered so the render graph knows
        // about it, but actual dispatch happens via the Renderer callback path.
        return true;
    }

    void MeshShaderRenderNode::Execute(CommandList*             cmdList,
                                       ...
                                       const Rect&               scissorRect) const
    {
        ...
        if (FAILED(cmdList->Get()->QueryInterface(IID_PPV_ARGS(&cmdList6)))) {
            DebugLog("MeshShaderRenderNode::Execute: ID3D12GraphicsCommandList6 not available.\n");
            return;
        }

        cmdList->SetGraphicsRootSignature(pipelineStateCache.GetMeshShaderRootSignature());
