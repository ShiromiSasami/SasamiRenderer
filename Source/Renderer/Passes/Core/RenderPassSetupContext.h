...
namespace SasamiRenderer
{
    ...
    struct RenderPassFrameInputs {
        RenderPassExecutionContext execution;
        RenderPassCameraData       camera;
        RenderPassShadowData       shadow;
        RenderPassLightingData     lighting;
        RenderPassGBufferData      gbuffer;
        RenderPassAoData           ao;
    ...
    public:
        ...
        RenderPassRequirements Build() const { return m_requirements; }

    private:
        RenderPassRequirements m_requirements{};
    };

    class RenderPassContextView
    {
    public:
        RenderPassContextView(const RenderPassExecutionPolicy* policy,
                              const RenderPassFrameInputs* inputs,
                              const RenderPassExecutionServices* services,
        ...
        const RenderPassExecutionPolicy& Policy() const { return *m_policy; }
        const RenderPassFrameInputs& Inputs() const { return *m_inputs; }
        const RenderPassExecutionServices& Services() const { return *m_services; }
        const RenderPassRequirements& Requirements() const { return m_requirements; }

    private:
        const RenderPassExecutionPolicy* m_policy = nullptr;
        const RenderPassFrameInputs* m_inputs = nullptr;
        const RenderPassExecutionServices* m_services = nullptr;
        RenderPassRequirements m_requirements{};
    };
}
