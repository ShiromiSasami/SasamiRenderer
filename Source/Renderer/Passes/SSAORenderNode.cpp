...
{
    void SSAORenderNode::BuildRequirements(RenderNodeRequirementBuilder& builder) const
    {
    ...
    void SSAORenderNode::Setup(RenderGraphBuilder& builder) const
    {
    ...
    bool SSAORenderNode::Execute(const RenderNodeContextView& context) const
    {
        if (!context.IsSatisfied())
        {
            DebugLog("SSAORenderNode::Execute: runtime context is invalid.\n");
            return false;
        }
    ...
    void SSAORenderNode::Execute(CommandList*              cmdList,
    ...
    void SSAORenderNode::ExecuteBlur(CommandList*              cmdList,
                                      RenderPipelineStateCache& pipelineStateCache,
                                      DescriptorHeap&           srvHeap,
                                      const Viewport&           viewport,
                                      ...
                                      D3D12_GPU_VIRTUAL_ADDRESS ssaoCbGpu) const
    {
        ...
        // We need: ssaoBlurTexture → RTV, depth → SRV (for edge-stopping bilateral).
        ...
        // Restore states: ssaoBlurTexture → SRV, depth → DEPTH_WRITE
        D3D12_RESOURCE_BARRIER endBarriers[2] = {};
        UINT endCount = 0;
