...
{
    void RayMarchRenderPass::BuildRequirements(RenderPassRequirementBuilder& builder) const
    {
    ...
    void RayMarchRenderPass::Setup(RenderGraphBuilder& builder) const
    {
    ...
    bool RayMarchRenderPass::Execute(const RenderPassContextView& context) const
    {
        if (!context.IsSatisfied()) {
            DebugLog("RayMarchRenderPass::Execute: runtime context is invalid.\n");
            return false;
        }
        ...
        if (!inputs.camera.invPv) {
            DebugLog("RayMarchRenderPass::Execute: cameraInvPV is null.\n");
            return false;
        }
        ...
        // extra0    -> debugMode, tanHalfFovX, tanHalfFovY, explicitCameraBasisEnabled
        ...
        const float explicitCameraBasisEnabled =
            (inputs.camera.mode == RenderCameraMode::RayMarch) ? 1.0f : 0.0f;
        const float extra0[4] = { m_debugMode, tanHalfFovX, tanHalfFovY, explicitCameraBasisEnabled };
        ...
        // Fullscreen triangle  Eno VB, no index buffer
