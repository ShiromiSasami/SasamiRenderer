...
namespace SasamiRenderer
{
    ...
    void Renderer::Render(const OverlayRenderCallback& overlay)
    {
        ...
        if (UsesNativeBackendFrame(*m_device)) {
            ...
            } else if (!m_sceneSubmitter.GetDrawItems().empty()) {
        ...
        const bool executeOpaqueFamilyPasses = (m_settings.rasterShaderMode == RasterShaderMode::Opaque);
        ...
        const RenderPassExecutionPolicy executionPolicy = BuildRenderPassExecutionPolicy(
        ...
        // The cross-queue fence sync is set up by RenderGraph::Execute()  Ethis submission
