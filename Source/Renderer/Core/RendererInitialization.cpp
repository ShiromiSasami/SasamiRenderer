...
// Renderer::Initialize — device creation, pipeline setup, resource allocation.
    ...
    bool Renderer::Initialize(HWND hWnd, UINT width, UINT height)
    {
...

        {
            ...
            if (UsesNativeBackendFrame(*m_device)) {
                ...
                DebugLog("Renderer::Initialize: native backend initialized with clear/present frame path. D3D12 render passes are not used on this backend yet.\n");
                return true;
            }
        }
        ...
        m_passRegistry.SetBuiltinNodes({
            ...
            m_transparentBackfaceDistanceRenderNode, m_transparentCompositeRenderNode
        });
