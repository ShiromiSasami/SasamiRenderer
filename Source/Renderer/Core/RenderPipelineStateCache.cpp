...
namespace SasamiRenderer
{
    ...
    bool RenderPipelineStateCache::Initialize(GraphicsDevice& device)
    {
        ...
        // SdfFluid, RayMarch, VolumetricCloud → InitializeEffectPipelines
        if (!InitializeEffectPipelines(device, vertexProfile, pixelProfile)) { return false; }

        // SSAO + SSAO Blur → InitializeSsaoPipelines
        ...
        if (!loadOrCompileShader({ L"SkinnedMesh_VS.hlsl", "VSMain", vertexProfile.c_str(), &skinnedVS })) {
            return false;
        }
