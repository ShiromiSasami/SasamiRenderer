...
namespace SasamiRenderer
{
    ...
    bool RendererFrameCoordinator::WaitForFrameFence(UINT frameIndex)
    {
        ...
        if (m_frameFence->GetCompletedValue() < fenceValue) {
            if (!m_frameFenceEvent ||
                FAILED(m_frameFence->SetEventOnCompletion(fenceValue, m_frameFenceEvent))) {
                return false;
            }
            const DWORD waitResult = WaitForSingleObject(m_frameFenceEvent, INFINITE);
    ...
    bool RendererFrameCoordinator::BeginFrame(UINT frameIndex, CommandList*& outCmdList)
    {
        ...
        if (!WaitForFrameFence(frameIndex)) {
            DebugLog("RendererFrameCoordinator::BeginFrame: frame fence wait failed.\n");
            return false;
        }
    ...
    void RendererFrameCoordinator::EnsureBoneBuffers(FrameContext& frame, UINT requiredCount)
    {
        ...
        // Each slot: 128 bones ÁE16 floats ÁE4 bytes = 8192 bytes (already 256-aligned)
