    ...
    bool SkinnedMeshBuffer::Upload(GraphicsDevice& device, const std::vector<SkinnedMesh>& meshes)
    {
        ...
        if (!device.GetCapabilities().supportsD3D12CompatibilitySurface &&
            ...
            for (const auto& m : meshes) {
                ...
                if (!m.vertices.empty()) {
                    const uint64_t vbBytes = static_cast<uint64_t>(sizeof(SkinnedVertex)) * m.vertices.size();
                    RhiBufferDesc vbDesc{};
                    vbDesc.sizeInBytes = vbBytes;
                    vbDesc.strideInBytes = sizeof(SkinnedVertex);
        ...
        for (const auto& m : meshes) {
            ...
            if (!m.vertices.empty()) {
                const UINT64 vbBytes = static_cast<UINT64>(sizeof(SkinnedVertex)) * m.vertices.size();

                D3D12_HEAP_PROPERTIES heapDefault{}; heapDefault.Type = D3D12_HEAP_TYPE_DEFAULT;
                D3D12_RESOURCE_DESC descVB{};
                descVB.Dimension = D3D12_RESOURCE_DIMENSION_BUFFER;
                descVB.Width = vbBytes; descVB.Height = 1; descVB.DepthOrArraySize = 1;
                descVB.MipLevels = 1; descVB.SampleDesc.Count = 1;
                descVB.Layout = D3D12_TEXTURE_LAYOUT_ROW_MAJOR;
                ...
                std::memcpy(pVB, m.vertices.data(), sizeof(SkinnedVertex) * m.vertices.size());
                vbUpload->Unmap(0, nullptr);

                uploadList.CopyBufferRegion(item.vb, 0, vbUpload, 0, vbBytes);
                auto vbBarrier = Transition(item.vb, D3D12_RESOURCE_STATE_COPY_DEST,
            ...
            if (!m.indices.empty()) {
                ...
                D3D12_HEAP_PROPERTIES heapDefault{}; heapDefault.Type = D3D12_HEAP_TYPE_DEFAULT;
                D3D12_RESOURCE_DESC descIB{};
                descIB.Dimension = D3D12_RESOURCE_DIMENSION_BUFFER;
                descIB.Width = ibBytes; descIB.Height = 1; descIB.DepthOrArraySize = 1;
                descIB.MipLevels = 1; descIB.SampleDesc.Count = 1;
                descIB.Layout = D3D12_TEXTURE_LAYOUT_ROW_MAJOR;
                ...
                auto ibBarrier = Transition(item.ib, D3D12_RESOURCE_STATE_COPY_DEST,
