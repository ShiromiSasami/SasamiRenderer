// GpuSoftwareRayTracerLegacy.cpp
                                                        ...
                                                        RayTracingRuntimeStats& outStats)
    {
...
#if defined(_DEBUG)
        D3D12_GPU_VIRTUAL_ADDRESS debugPtLightsVA = 0;
        bool debugLightBufValid = m_lightDataBuffer.IsValid();
#endif
        ...
        DebugIncrementDispatchCount();
        {
            ...
            snprintf(buf, sizeof(buf),
                "[DispatchDiag] idx=%u pass=Legacy_ReflectionFallback lightBufValid=%d lightVA=0x%llX\n",
                DebugGetDispatchCount(), debugLightBufValid ? 1 : 0,
                (unsigned long long)debugPtLightsVA);
            DebugLog(buf);
        }
#endif
        cl->Dispatch(gx, gy, 1u);
